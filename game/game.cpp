...
public:
  void FirstBoot(Player &player) {
    ...
    while (ip.good()) {
      ...
      content.replace(content.find("$player_name"), sizeof("$player_name") - 1,
                      player.name);
  ...
  void sellerMarketBoot(Player &player) {
    ...
    showMessage("For a reasonable cost, ofcourse!",
                "So make sure you have a bank account that I can pay you to..",
                2, name);
    sleep(2);
    achievementMessage("Selling Unlocked");
    achievementMessage("Bank Unlocked");
    sleep(2);
    showMessage("Or you could just show up to have a conversation...",
...
void gamble(Player &player) {
  ...
  while (true) {
    ...
    if (pf % 2 == 0) { // 50% chance only - add more randomness.... TO DO
      // Profit:
      cout << "You made " << moneyAsString(profit) << " Profit for "
