...
void showDialogue(string author = "", string content="", int timeout = 1)
{
    if (author != "")
    {
        cout << author << ": ";
...
class Inventory
{
    ...
    //Fishing items:
    item soakedBoot = {"Soaked Boots", 0, 0.82, 0.79, 1.38};
    item usedEarphones = {"Used earphones (white", 0, 0.87, 0.08, 1.68};
    item usedCondom = {"Used condom (ew)", 0, 0.89, 0, 0.1};
    item salmon = {"Salmon", 0, 0.75, 4.50, 2.79};
    item clownfish = {"Clown Fish", 0, 0.74, 5.50, 2.95};
    item tuna = {"Tuna", 0, 0.63, 4.99, 3.11};
    item trout = {"Trout", 0, 0.43, 7.85, 5.32};
    item squid = {"Squid", 0, 0.35, 15.85, 11.18};
    item octopusLeg = {"Octopus Leg", 0, 0.21, 37.48, 21.11};
    item jellyfish = {"Jellyfish", 0, 0.18, 118.21, 56.5};
    item lobser = {"Lobster", 0, 0.17, 150.00, 69.69};
    ...
    {
        vector<item> items = {soakedBoot, usedEarphones, usedCondom, salmon, clownfish, tuna, trout, squid, octopusLeg, jellyfish, lobser, whaleTooth};
        return items;
    }
...
class Mine
{
    ...
    vector<item*> getMineable(Player &player)
    {
        ...
        vector<item*> items = player.getMineItemsByAddress();  // vector<item> items = {dirt, rock, wood, coal, granite, iron, copper, silver, tin,   hardRock, gold, diamond, ruby, blackStone, magma, bedrock}; vector<item*> items = {&dirt, &rock, &wood, &coal, &granite, &iron, &copper, &silver, &tin, &hardRock, &gold, &diamond, &ruby, &blackStone, &magma, &bedrock};
...
class RailwayStation
{
    ...
    void boardTrain(Player &player, ticket tx)
    {
       ...
       // vector<string> travelMessages = {"Breeze feeling good...", "Watching moving trees...", "Stretching legs...", "Leaning back the recliner...", "Listening to music...", "Looking out the window...", "Rolling down the AC..."};
        ...
        announcement << tx.Train.name << " arrived at platform " << randInt(1, tx.Train.destination.platformCount) << "!";
        showDialogue("ANNOUNCER", announcement.str()); // dynamic messages frfr should be done TODO!!!
        player.currentCity = tx.Train.destination;
        sleep(1);
        cout << "De-boarding train..." << endl;
        sleep(2);
        cout << "You have reached " << player.currentCity.name << "! Happy Journey!" << endl;
        sleep(1);
    ...
    bool enterPlatform(Player &player, int platformNumber, ticket tx)
    {
        ...
        if(tx.Train.platform != platformNumber)
        {
            ...
            cout << "[go to the platform before time runs out! you have " << maxTime - platformEnterTime << " seconds left!]" << endl;
            sleep(1);
            cout << ">> ";
            clearInputBuffer();
            cin.get();
        ...
        {
            cout << "Too late, you missed your train while being on the wrong platform!" << endl;
            platformEnterTime = 2;
            maxTime = 0;
            init();
            sleep(3);
        ...
        int countdown = tx.Train.waitTime; // countdown starting value
            ...
            ostringstream announcement;
            cout << endl;
            announcement << tx.Train.name << " has arrived on platform " << tx.Train.platform << "!";
    ...
    void enter(Player &player)
    {
        ...
        while(true)
        {
            ...
            {
                int platformNumber;
                showDialogue("Security guard", "There you go... get on the right platform! Don't miss your train...");
                while(true)
                {
                    // cout << "1) Platform 1\n2) platform 2\n3) Platform 3\n0) Quit\n>> ";
            ...
            {
                showDialogue("Security guard", "You can't enter without a train ticket. Show me a valid ticket, man.");
            }
